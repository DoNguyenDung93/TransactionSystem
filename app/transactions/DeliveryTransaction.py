...
class DeliveryTransaction(Transaction):
    def execute(self, params):
        ...
        self.get_order_sum_query = self.session.prepare('select ol_amount from order_line where o_id = ?')
    ...
    def find_customer(self, smallest_order_number):
        result = self.session.execute(self.find_customer_query.bind(smallest_order_number))
    ...
    def update_order(self, smallest_order_number, carrier_id):
        self.session.execute(self.update_order_query.bind(carrier_id, smallest_order_number))
    ...
    def update_order_line(self, smallest_order_number):
        ...
        self.session.execute(self.update_order_line_query.bind(
            time.strftime('%Y-%m-%d %H:%M:%S'), smallest_order_number))

    # Get the sum of value of all the items in the order
    def get_order_sum(self, smallest_order_number):
        result = self.session.execute(self.get_order_sum_query.bind(smallest_order_number))
        sum_order = 0.0
    ...
    def update_customer_balance(self, customer_id, sum_order):
        self.session.execute(self.update_customer_balance_query.bind(sum_order, customer_id))

    # Increase the number of delivery to the customer by 1
    def update_customer_delivery_cnt(self, customer_id):
        self.session.execute(self.update_customer_delivery_cnt_query.bind(customer_id))
